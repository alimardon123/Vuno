// Extensions: what this org can do, as opposed to who is in it.
//
// Three sections, the same three Claude Code settings has, because they are the
// three real kinds of thing and people already know the words: a **skill** is
// what a member knows, a **connector** is what a member can reach, a **plugin**
// is a package that installs both and hires whoever uses them.
//
// This is the seventh rail destination, and the rule in `rail.tsx` says a
// seventh usually belongs inside one that exists. It does not here: Members
// answers "who is in this org", and Extensions answers "what can this org do".
// Putting the second inside the first made the Library a tab people had to know
// to look for (docs/IA-NAVIGATION.md).
      ...
      <header className="sticky top-0 z-10 border-b border-[var(--line)] bg-[var(--surface)] px-6 py-3">
        <div className="mx-auto w-full max-w-[70rem]">
          ...
          <p className="mt-0.5 text-[11.5px] text-[var(--fg-3)]">
            What this org can do. A skill is what a member knows, a connector is what they can reach, and a
            plugin installs both and hires whoever uses them.
          </p>
          <nav className="mt-2 flex gap-1" aria-label="Extensions view">
            {TABS.map(([id, label]) => (
              <Link
                key={id}
                href={id === 'skills' ? '/extensions' : `/extensions?tab=${id}`}
